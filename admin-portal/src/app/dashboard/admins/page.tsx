...
export default function AdminsPage() {
    ...
    const checkAccessAndFetch = async () => {
        try {
            ...
            // Use API to check admin status (bypasses RLS)
            ...
            if (!adminCheckData.exists || !adminCheckData.admin || adminCheckData.admin.role !== 'super_admin') {
                router.push('/dashboard');
                return;
            }

            setCurrentAdmin(adminCheckData.admin);
            fetchAdmins();
        } catch {
            router.push('/dashboard');
        }
    ...
    const toggleAdminStatus = async (admin: Admin) => {
        ...
        try {
            ...
            const response = await fetch('/api/admins/freeze', {
                ...
                body: JSON.stringify({
                    adminId: admin.id,
                    freeze: admin.is_active, // Toggling active means freezing
                }),
            });
